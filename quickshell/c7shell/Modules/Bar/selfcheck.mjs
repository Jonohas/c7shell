...
const Pips = load("../../Common/Pips.js", ["layout", "window"]);

// -- faces and scatters: the dot count IS the workspace number --
for (let n = 1; n <= 12; n++) {
  const l = Pips.layout(n);
  assert.equal(l.dots.length, n, `layout(${n}) dot count`);
  assert.equal(l.divider, false, `layout(${n}) has no divider`);
...
assert.equal(Pips.layout(7).dotSize, 2.5, "scatter faces use 2.5px dots");
...
// -- scatter faces: six on an even outer ring, the rest in the middle --
const radius = ([x, y]) => Math.hypot(x - 0.5, y - 0.5);
for (let n = 7; n <= 12; n++) {
  const dots = Pips.layout(n).dots;
  for (const d of dots.slice(0, 6))
    assert.ok(Math.abs(radius(d) - 0.32) < 1e-9, `layout(${n}) outer ring radius`);
  for (const d of dots.slice(6))
    assert.ok(radius(d) <= 0.16 + 1e-9, `layout(${n}) inner dots stay in the middle`);
}
